...
export default function HeroSection() {
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      // p = 0 at page top (Group1, arms far apart)
      // p = 1 at end of scroll zone (Group2, arms close / touching)
      const p = Math.min(Math.max(-rect.top / scrollable, 0), 1);

      // ─── Adam (bottom-left arm) ───
      // Group1 start: far bottom-left, outside viewport almost
      // Group2 end:   moved right/up so hand meets center
  ...
  return (
    // 300vh tall — gives scroll room while sticky viewport holds
